...
public class CobwebFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (int i = 64; i < world.getHeight(); i++) {
            ...
            if (world.getBlockState(mutable).is(Blocks.SPRUCE_LEAVES) && world.getBlockState(mutable.below()).isAir() && random.nextBoolean() && FeatureHelper.isCorrectBiome(world.getBiomeName(mutable).get())) {
